...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[
        hubnode/.style={cylinder, draw=nustblue, fill=nustlight, shape aspect=0.4, line width=1.1pt, minimum width=3.2cm, minimum height=2.0cm, align=center, font=\small\bfseries},
        appnode/.style={rectangle, rounded corners=3pt, draw=black, fill=white, line width=0.9pt, text width=4.2cm, minimum height=1.35cm, align=center, font=\small},
        linkarrow/.style={latex-latex, line width=1.0pt, color=nustblue}
    ]
        % Central Hub
        \node[hubnode] (hub) at (0, 0) {CENTRAL DATABASE\\ \scriptsize Unified Firebase Core\\ \tiny \texttt{/dna\_exports} \& \texttt{/transactions}};
        
        % 4 Surrounding Apps
        \node[appnode] (signet) at (-5.5, 2.1) {\textbf{1. DNAx (Signet)}\\ \footnotesize Bioinformatic Design Suite\\ \tiny Sequence \& Assay Generation};
        \node[appnode] (inv) at (5.5, 2.1) {\textbf{2. DNAx Inventory}\\ \footnotesize Desktop Warehouse Terminal\\ \tiny Batch Generator \& Scanner};
        \node[appnode] (forensic) at (-5.5, -2.1) {\textbf{4. DNAx Forensics}\\ \footnotesize Desktop Forensic Lab App\\ \tiny Sequence, Gel \& Barcode Tests};
        \node[appnode] (vendor) at (5.5, -2.1) {\textbf{3. DNAx Vendor}\\ \footnotesize Mobile Android/iOS App\\ \tiny Issuance \& Custody Transfer};
