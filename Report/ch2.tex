...
\begin{enumerate}
    ...
    \item \textbf{Standard PCR Master Mix ($25~\mu\text{L}$ Volume)}: Reagents were combined in $0.2\text{ mL}$ tubes comprising $2.5~\mu\text{L}$ 10$\times$ PCR buffer, $1.5~\mu\text{L}$ $\text{MgCl}_2$, $1.0~\mu\text{L}$ dNTPs, $1.0~\mu\text{L}$ Taq DNA polymerase, $1.0~\mu\text{L}$ each of forward and reverse primers, $1.0\text{--}2.0~\mu\text{L}$ template DNA, and $15.0\text{--}16.0~\mu\text{L}$ distilled water.
...
\begin{itemize}
    \item \textbf{Probe-Based qPCR Formulation ($20~\mu\text{L}$ Volume)}: Reagents comprise $4.0~\mu\text{L}$ of 2$\times$ Super Real Probe Premix, $0.6~\mu\text{L}$ forward primer, $0.6~\mu\text{L}$ reverse primer, $0.4~\mu\text{L}$ fluorophore probe, $1.0\text{--}2.0~\mu\text{L}$ template DNA, and $12.4\text{--}13.4~\mu\text{L}$ RNase-free $\text{ddH}_2\text{O}$.
    \item \textbf{Thermal Cycling \& Melt Analysis}: Initial hot-start activation at $95^\circ\text{C}$ for 15~min, followed by 40 cycles of denaturation ($95^\circ\text{C}$, 30s), annealing ($55^\circ\text{C}$, 30s), extension ($72^\circ\text{C}$, 45s), hold at $95^\circ\text{C}$ for 60s, and high-resolution melt acquisition from $72^\circ\text{C}$ to $95^\circ\text{C}$ at $0.3^\circ\text{C}/\text{s}$.
